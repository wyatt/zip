...
function altitudeProfile(points,terrain,options){
  ...
  const altitude=points.map(p=>terrain.heightAt(p,radius)+margin);
  for(let i=altitude.length-2;i>=0;i--){const rise=distance2(points[i],points[i+1])*options.climbSpeed/options.speed;altitude[i]=Math.max(altitude[i],altitude[i+1]-rise);}
  for(let i=1;i<altitude.length;i++){const drop=distance2(points[i-1],points[i])*options.climbSpeed/options.speed;altitude[i]=Math.max(altitude[i],altitude[i-1]-drop);}
...
export async function planRegionalMission({regionUrl,config,progress=()=>{}}){
  ...
  const profiled=altitudeProfile(route,terrain,options),surface=terrain.heightAt(config.a,0),start=[config.a[0],config.a[1],surface],
