...
async function loadAerial(url, tileId) {
  try {
    return await loadJpeg(`${url}tiles/${tileId}/aerial.jpg`);
  } catch {
    return await loadJpeg(`${url}tiles/${tileId}/aerial-5m.jpg`);
  }
}
...
export async function createRegionalReconstruction(regionUrl, manifest, plan, onProgress = () => {}) {
  ...
  const resolution = Math.max(1, Math.ceil(span / 1200));
  ...
  const data = {
    ...
    sources: { color: "NYS spring 2023 1 m orthoimagery", elevation: "2020 maximum-return display terrain", note: "Simulated capture reveal; not photogrammetric reconstruction" },
  };
  const seen = new Set();
