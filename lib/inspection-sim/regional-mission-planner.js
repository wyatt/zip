...
function altitudeProfile(points,terrain,options,holdHighUntil=0){
  ...
  const end=Math.min(Math.max(0,holdHighUntil),altitude.length-1);
  if(end>0){
...
export async function planRegionalMission({regionUrl,config,progress=()=>{}}){
  ...
  const profiled=altitudeProfile(route,terrain,options,holdHighUntil),surface=terrain.heightAt(config.a,0),start=[config.a[0],config.a[1],surface],
