...
export function validateAdapterPlan(plan: FlightPlan, identity: AdapterIdentity, adapter: DroneAdapter) {
  ...
  if (!Array.isArray(plan.steps) || plan.steps.length < 3 || plan.steps.length > 54 || plan.steps[0].kind !== "takeoff" || plan.steps.at(-1)?.kind !== "land") throw new Error("Flight plan must begin with takeoff and end with landing.");
    if (![plan.radiusM, plan.maxAltitudeM, plan.maxDurationSec, plan.minimumBatteryPct, plan.cruiseSpeedMps].every(Number.isFinite) || plan.radiusM < 5 || plan.radiusM > 10000 || plan.maxAltitudeM < 2 || plan.maxAltitudeM > 200 || plan.maxDurationSec <= 0 || plan.maxDurationSec > 7200 || plan.minimumBatteryPct < 10 || plan.minimumBatteryPct > 100 || plan.cruiseSpeedMps <= 0) throw new Error("Invalid flight plan limits.");
  ...
  for (const [index, step] of plan.steps.entries()) {
    ...
    if (!["takeoff", "waypoint", "hover", "land", "survey", "deliver", "search"].includes(step.kind) || !Number.isFinite(step.altitudeM) || !Number.isFinite(step.durationSec) || step.durationSec < 0 || step.altitudeM < 0 || step.altitudeM > plan.maxAltitudeM || metersBetween(plan.home, step.position) > plan.radiusM) throw new Error("Flight step exceeds the plan's limits.");
    ...
    if ((step.kind === "survey" || step.kind === "deliver" || step.kind === "search") && typeof adapter.beginRegionalTask !== "function") throw new Error("Aircraft adapter does not support regional demo missions.");
