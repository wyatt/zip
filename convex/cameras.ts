...
export const publish = mutation({ args: { token: v.string(), sessionId: v.id("agentSessions"), operationId: v.id("operations"), protocol: v.union(v.literal("whep"), v.literal("hls")), url: v.string(), expiresAt: v.number() }, handler: async (ctx, args) => {
  const { vehicle } = await requireAgentSession(ctx, args.token, args.sessionId);
  const operation = await ctx.db.get(args.operationId);
  if (!operation || operation.vehicleId !== vehicle._id || !vehicle.capabilities.includes("camera") || ["cancelled", "completed"].includes(operation.state)) throw new Error("Camera is not available for this operation.");
  ...
  if (url.username || url.password || (url.protocol !== "https:" && !(vehicle.environment === "simulated" && url.protocol === "http:" && ["127.0.0.1", "localhost"].includes(url.hostname)))) throw new Error("Camera transport must use HTTPS.");
