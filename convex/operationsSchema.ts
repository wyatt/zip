...
export const operationsTables = {
  ...
  vehicles: defineTable({ operatorId: v.id("users"), name: v.string(), model: v.optional(v.string()), hardwareId: v.string(), environment, capabilities: v.array(capability), maxPayloadKg: v.number(), cameraMp: v.optional(v.number()), home: geo, launchSiteId: v.optional(v.string()), launchSiteName: v.optional(v.string()), maxRadiusM: v.number(), available: v.boolean(), integrationApproved: v.boolean(), controlGeneration: v.optional(v.number()), activeOperationId: v.optional(v.id("operations")), activeSessionId: v.optional(v.id("agentSessions")) }).index("by_operator", ["operatorId"]).index("by_available", ["available"]).index("by_hardwareId", ["hardwareId"]),
