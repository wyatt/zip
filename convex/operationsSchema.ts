...
export const presetLocation = v.object({ name: v.string(), lat: v.number(), lon: v.number() });
...
export const operationsTables = {
  ...
  vehicles: defineTable({ operatorId: v.id("users"), name: v.string(), model: v.optional(v.string()), hardwareId: v.string(), environment, capabilities: v.array(capability), maxPayloadKg: v.number(), cameraMp: v.optional(v.number()), home: geo, maxRadiusM: v.number(), available: v.boolean(), integrationApproved: v.boolean(), controlGeneration: v.optional(v.number()), activeOperationId: v.optional(v.id("operations")), activeSessionId: v.optional(v.id("agentSessions")) }).index("by_operator", ["operatorId"]),
