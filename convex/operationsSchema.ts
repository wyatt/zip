...
export const presetLocation = v.object({ id: v.optional(v.string()), name: v.string(), lat: v.number(), lon: v.number() });
...
export const operationsTables = {
  members: defineTable({ userId: v.id("users"), displayName: v.string(), role: v.union(v.literal("customer"), v.literal("operator"), v.literal("demo"), v.literal("admin")) }).index("by_user", ["userId"]),
  operatorProfiles: defineTable({ userId: v.id("users"), approved: v.boolean(), acceptingJobs: v.boolean(), qualifications: v.array(jobKind), base: geo, serviceRadiusM: v.number(), presetLocations: v.optional(v.array(presetLocation)), activeOperationId: v.optional(v.id("operations")), lifetimeEarningsCents: v.optional(v.number()) }).index("by_user", ["userId"]),
  ...
  vehicles: defineTable({ operatorId: v.id("users"), name: v.string(), model: v.optional(v.string()), hardwareId: v.string(), environment, capabilities: v.array(capability), maxPayloadKg: v.number(), cameraMp: v.optional(v.number()), home: geo, launchSiteId: v.optional(v.string()), launchSiteName: v.optional(v.string()), maxRadiusM: v.number(), available: v.boolean(), integrationApproved: v.boolean(), batteryPct: v.optional(v.number()), controlGeneration: v.optional(v.number()), activeOperationId: v.optional(v.id("operations")), activeSessionId: v.optional(v.id("agentSessions")) }).index("by_operator", ["operatorId"]).index("by_available", ["available"]).index("by_hardwareId", ["hardwareId"]),
  agentCredentials: defineTable({ operatorId: v.optional(v.id("users")), vehicleId: v.optional(v.id("vehicles")), tokenHash: v.string(), createdAt: v.number(), expiresAt: v.number(), revokedAt: v.optional(v.number()), lastSeenAt: v.optional(v.number()) }).index("by_hash", ["tokenHash"]).index("by_operator", ["operatorId"]).index("by_vehicle", ["vehicleId"]),
  ...
  cameraSessions: defineTable({ operationId: v.id("operations"), vehicleId: v.id("vehicles"), sessionId: v.id("agentSessions"), protocol: v.union(v.literal("hls"), v.literal("whep"), v.literal("mjpeg")), url: v.string(), storageId: v.optional(v.id("_storage")), expiresAt: v.number() }).index("by_operation", ["operationId"]),
